...
export function MarketingLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-canvas text-ink font-sans">
      <header className="sticky top-0 z-40 bg-canvas/80 backdrop-blur-md border-b border-ink/8">
        <div className="mx-auto max-w-6xl px-5 md:px-8 h-16 flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center gap-7">
            {NAV.map((n) => (
              <Link
                key={n.to}
                to={n.to}
                className="t-body-sm text-ink/75 hover:text-ink transition-colors font-medium"
          ...
          <div className="flex items-center gap-2">
            <Link
              to="/download"
              className="hidden sm:inline-flex t-button items-center rounded-full bg-ink text-canvas px-5 py-2 hover:opacity-90 transition shadow-sm font-semibold text-xs uppercase tracking-wider"
            >
              Get App
            </Link>
            <button
      ...
      <footer className="border-t border-ink/8 bg-[#f9f7f2] mt-24">
        <div className="mx-auto max-w-6xl px-5 md:px-8 py-14 grid gap-8 md:grid-cols-4">
          <div className="md:col-span-2">
            <div className="flex items-center gap-2">
              <span className="inline-grid place-items-center size-9 rounded-2xl bg-[#FF671F] text-white font-bold select-none text-lg">M</span>
              <span className="t-title text-lg font-bold tracking-tight text-ink">MinDrop</span>
            </div>
            <p className="t-body-sm text-ink/75 mt-3 max-w-sm leading-relaxed">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {NAV.map((n) => (
                <li key={n.to}>
                  <Link to={n.to} className="t-body-sm text-ink/85 hover:text-ink font-medium transition-colors">{n.label}</Link>
                </li>
              ))}
              <li><Link to="/download" className="t-body-sm text-ink/85 hover:text-ink font-medium transition-colors">Download</Link></li>
            </ul>
          </div>
          <div>
            <p className="t-eyebrow text-ink/50 mb-3 uppercase tracking-wider font-semibold">Legal & Support</p>
            <ul className="space-y-2">
              <li><Link to="/privacy" className="t-body-sm text-ink/85 hover:text-ink font-medium transition-colors">Privacy Policy</Link></li>
              <li><Link to="/terms" className="t-body-sm text-ink/85 hover:text-ink font-medium transition-colors">Terms of Service</Link></li>
              <li><Link to="/refunds" className="t-body-sm text-ink/85 hover:text-ink font-medium transition-colors">Refund Policy</Link></li>
              <li><Link to="/contact" className="t-body-sm text-ink/85 hover:text-ink font-medium transition-colors">Contact Support</Link></li>
            </ul>
          </div>
        </div>
        <div className="border-t border-ink/8">
          <div className="mx-auto max-w-6xl px-5 md:px-8 py-5 t-meta text-ink/50 flex flex-wrap justify-between gap-2">
