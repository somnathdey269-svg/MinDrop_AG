...
import { 
  ...
  Layers, Volume2, ShieldCheck, Heart, Home, Key
} from "lucide-react";
...
function ShowcaseDeckPage() {
  ...
  return (
    <div className="fixed inset-0 bg-canvas text-ink font-sans flex flex-col justify-between p-6 select-none overflow-hidden h-[100dvh] w-screen">
      ...
      <header className="flex justify-between items-center w-full z-30">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-black uppercase tracking-wider hidden sm:inline">MinDrop — The Web Can Do What!?</span>
        </div>
      ...
      {/* 2. Central Presentation Card Deck */}
      <div className="flex-1 flex items-center justify-center relative w-full my-4">
        ...
        <div className="relative w-[300px] sm:w-[360px] md:w-[400px] h-[400px] sm:h-[460px] flex items-center justify-center z-10">
          <AnimatePresence mode="popLayout">
            {/* The Behind/Next Card Preview */}
            <motion.div
              key={`next-${nextCard.id}`}
              initial={{ scale: 0.9, rotate: 6, opacity: 0.8 }}
              animate={{ scale: 0.94, rotate: 8, opacity: 0.9 }}
              exit={{ opacity: 0 }}
              className={`absolute inset-0 rounded-[2.5rem] border-3 border-ink p-8 flex flex-col justify-between bg-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] pointer-events-none select-none`}
            >
              <div>
      ...
      {/* 3. Footer Corners */}
      <footer className="flex justify-between items-center w-full z-30">
        <div className="text-xs font-black uppercase tracking-wider text-ink/50">
          Chrome for Developers (Inspired)
        </div>

        {/* Center Toggle Control */}
        <div className="flex items-center gap-1 bg-white border-2 border-ink rounded-full px-3 py-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
          {DECK_CARDS.map((card, idx) => (
            <button
              key={card.id}
              onClick={() => setActiveIdx(idx)}
              className={`size-2.5 rounded-full border border-ink transition-colors cursor-pointer ${
                idx === activeIdx ? "bg-[#FF671F]" : "bg-ink/10"
              }`}
          ...
          <button
            ...
            className="ml-2 cursor-pointer p-0.5 hover:text-[#FF671F]"
            aria-label="Autoplay"
          >
            <Play className={`size-3 ${autoPlay ? "fill-[#FF671F] stroke-[#FF671F]" : "text-ink"}`} />
          </button>
        </div>
          ...
          <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                ...
                <span className="inline-flex items-center gap-1.5 rounded-full border border-ink/15 bg-[#FFFBEB] px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[#F59E0B] mb-4">
                  💡 Project Info
                </span>

                <h3 className="text-3xl font-black text-ink leading-tight">About MinDrop</h3>
                <p className="text-xs text-ink/75 font-semibold mt-4 leading-relaxed">
                  MinDrop is a offline second memory designed by creators who struggle with daily forgetfulness. 
                  Unlike busy, overwhelming task managers, MinDrop focus on immediate micro-actions—looping alarms for small tasks, location sweep reminders, and app notification filters.
                </p>

                <p className="text-xs text-ink/65 font-medium mt-3 leading-relaxed">
                  Inspired by Google's Web Showcase design language, this page is built fully statically on Cloudflare Pages using React, Tailwind V4, and TanStack start.
                </p>
