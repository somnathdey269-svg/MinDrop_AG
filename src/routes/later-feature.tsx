...
function LaterFeatureDetailView() {
  ...
  const { page, hasBlocks } = useCMSPage("later-feature");

  if (hasBlocks && page && page.blocks && page.blocks.length > 0) {
    return (
      <div className="min-h-screen bg-[#D1FAE5] flex flex-col select-none">
        <header className="shrink-0 h-14 border-b-2 border-[#10B981]/10 z-50 bg-[#D1FAE5]/96 backdrop-blur-md px-4 sm:px-6 flex items-center justify-between">
          <Link to="/" resetScroll={true} viewTransition className="flex items-center gap-1 text-xs font-black uppercase text-[#047857]">
            <X className="size-3.5"/> Close
          </Link>
          <MinDropHeaderLogo className="text-lg sm:text-2xl shrink-0" />
          <Link to="/download" resetScroll={true} viewTransition className="px-3.5 py-1.5 rounded-full bg-[#10B981] text-white text-xs font-black uppercase shadow-md">
            Get App
          </Link>
        </header>
        ...
        {slides.map((slide, idx) => (
          <section key={idx} className="w-full h-full shrink-0 snap-start snap-always flex items-center justify-center relative overflow-hidden perspective-[1200px]">
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center overflow-hidden">
              <div className={`w-[450px] h-[450px] rounded-full blur-3xl opacity-20 animate-pulse ${isDark ? "bg-emerald-400" : "bg-[#10B981]"}`} />
            </div>
            <motion.div
